#' @title hidimSurv
#' Survival analysis on high dimensional data
#'
#' @description Survival analysis using Cox Proportional hazards function on high dimensional data
#'
#' @details hidimSurv function first creates a new column 'Status' in the input
...
#' @seealso hidimSurvbon hidimSurvbonlas hidimsvc


hidimSurv <- function(m,n,siglevel,ID,OS,Death,PFS,Prog,data){
  ...
  report3 <- list()
  report3[[1]] <- "Estimated values for significant variables on OS and survival event"
  report3[[2]] <- significantpvalueA
  print(report3)

  report4 <- list()
  report4[[1]] <- "Estimated values for significant variables on PFS and progression event"
  ...
  output<-list(report1,report2,report3,report4,report5,report6,report7)
  return(output)
